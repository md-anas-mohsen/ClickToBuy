...
const Header = () => {
  ...
  const categoriesNum = isLG ? 6 : isMD ? 3 : 0;
            ...
            {categories.slice(0, categoriesNum).map((c) => (
              <MenuItem
                ...
                onClick={() => {
                  ...
                  history.push(`/search?category=${c.category_name}`);
                }}
                className={
                  category === c.category_name ? classes.activeCategory : ""
                }
              >
                {c.category_name}
              </MenuItem>
            ))}
            {categories.slice(0, categoriesNum).length === categoriesNum && (
              <TemporaryDrawer
